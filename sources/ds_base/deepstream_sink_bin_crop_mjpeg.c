...
/**
 * Function to create sink bin to generate encoded video file output.
 */
static gboolean
create_encode_file_bin (NvDsSinkEncoderConfig * config, NvDsSinkBinSubBin * bin)
{
  printf("---------------- Using modified create_encode_file_bin\n");
                      ...
                      NvDsEncoderType enctype, guint64 udp_buffer_size)
{
  ...
  printf("udp_buffer_size was %lu\n", udp_buffer_size);
  if (udp_buffer_size == 0)
    udp_buffer_size = 512 * 1024 * 8;
...
{
  printf("---------- Modified create_udpsink_bin\n");
  GstCaps *caps = NULL;
  gboolean ret = FALSE;
  gchar elem_name[50];
  ...
  if (config->enc_type == NV_DS_ENCODER_TYPE_SW || config->codec == 4)
    caps = gst_caps_from_string ("video/x-raw, format=I420");
  else
    caps = gst_caps_from_string ("video/x-raw(memory:NVMM), format=I420");
  ...
  switch (config->codec) {
    ...
    case 4:
      printf("--------- RAW\n");
      bin->codecparse = NULL; // gst_element_factory_make ("h265parse", "h265-parser");
//      g_object_set (G_OBJECT (bin->codecparse), "config-interval", -1, NULL);
      bin->rtppay = gst_element_factory_make ("rtpvrawpay", rtppay_name);
      bin->encoder = NULL;
      break;
    default:
      goto done;
...
//  }
  ...
  {
      gst_bin_add_many (GST_BIN (bin->bin),
          bin->queue, bin->transform, bin->cap_filter,
          bin->rtppay, bin->sink, NULL);

      NVGSTDS_LINK_ELEMENT (bin->queue, bin->transform);
      NVGSTDS_LINK_ELEMENT (bin->transform, bin->cap_filter);
      NVGSTDS_LINK_ELEMENT (bin->cap_filter, bin->rtppay);
//      NVGSTDS_LINK_ELEMENT (bin->queue, bin->cap_filter);
//      NVGSTDS_LINK_ELEMENT (bin->cap_filter, bin->transform);
//      NVGSTDS_LINK_ELEMENT (bin->transform, bin->rtppay);
      NVGSTDS_LINK_ELEMENT (bin->rtppay, bin->sink);
  }
...
create_demux_sink_bin (guint num_sub_bins, NvDsSinkSubBinConfig * config_array,
    NvDsSinkBin * bin, guint index)
{
printf("--------------- in create_demux_sink_bin\n");
